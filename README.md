...
1. **Buffer de Mensagens (Redis):** Implementação de fila para garantir que mensagens recebidas fora de ordem (comum em redes móveis) sejam processadas na sequência correta (FIFO).
2. **Memória de Conversa (Sliding Window):** O sistema mantém apenas o contexto relevante das últimas interações no Redis, otimizando custos de tokens e latência.
3. **Segurança de Transação:** Utilização de *Row-Level Locking* no PostgreSQL para impedir que dois clientes agendem o mesmo horário simultaneamente durante picos de acesso.
